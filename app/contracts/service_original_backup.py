...
class ContractService:
    ...
    async def list_contracts(self, db=None) -> Dict[str, Any]:
        ...
        try:
            if not db:
                # Si no se proporciona conexión, usar la función original del sistema de archivos
                return await self._list_contracts_from_filesystem()
            
            # Usar la función de base de datos
            query = "SELECT * FROM public.sp_get_dashboard_contracts_full()"
            result = await db.fetch_all(query)
            
            contracts = []
            for row in result:
                # Convertir el resultado de la función a diccionario
            ...
            print(f"Error obteniendo contratos de la base de datos: {e}")
            # Fallback al sistema de archivos si hay error en la BD
            return await self._list_contracts_from_filesystem()

    async def _list_contracts_from_filesystem(self) -> Dict[str, Any]:
        """Listar contratos desde el sistema de archivos (método original)"""
        contracts = []

        if not self.contracts_dir.exists():
            return {"success": True, "contracts": [], "total": 0}
